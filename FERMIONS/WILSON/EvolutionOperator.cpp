...
namespace EvolutionOperator{
    ...
    void ConvertGaugeLinks(GaugeLinks *U,u_cc *Ucc){
                    ...
                    for(INT mu=0;mu<U->Dimension;mu++){
                        ...
                        for(INT i=0;i<Nc;i++){
                            for(INT j=0;j<Nc;j++){
                                ...
                                Ucc[CCIndex].c[i].c[j]=UMat[i+j*Nc];
    ...
    // INCLUDE WILSON OPERATOR //
    #include "OPERATOR/WilsonOperator.cpp"
    
    // COMPUTE \partial_{t} Psi= -i Gamma0 [ -i DSlash_s +m] Psi //
    void ComputeSingleMode(FermionField *Psi,INT sIndex,wcf *DerivativePsi){
        
        //c0=clock();

        WilsonEvoOperator::Compute(Ucc,Psi->GetMode(sIndex),DerivativePsi);
     
        //c1=clock();
     
        //printf("the elapsed cpu time in calculating wilson vector: %f\n",(float)(c1-c0)/CLOCKS_PER_SEC);
